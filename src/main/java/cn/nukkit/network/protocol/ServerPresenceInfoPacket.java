...
public class ServerPresenceInfoPacket extends DataPacket {
    ...
    public void decode() {
        this.presenceConfiguration = this.getOptional(null, (s) -> {
            // v1001 引入 optional 名称 + richPresenceId；v2168 起仅剩 optional richPresenceId
            // v1001 added optional names + richPresenceId; since v2168 only an optional richPresenceId remains
            if (this.protocol >= ProtocolInfo.v1_26_40) {
                String richPresenceId = s.getOptional(null, BinaryStream::getString);
                return new PresenceConfiguration(null, null, richPresenceId);
            }
    ...
    public void encode() {
        ...
        this.putOptionalNull(this.presenceConfiguration, (s) -> {
            if (this.protocol >= ProtocolInfo.v1_26_40) {
                this.putOptionalNull(s.getRichPresenceId(), this::putString);
            } else if (this.protocol >= ProtocolInfo.v1_26_30) {
