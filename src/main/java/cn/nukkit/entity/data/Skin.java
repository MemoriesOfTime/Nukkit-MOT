...
public class Skin {
    ...
    public byte[] skinBytes; // 0.14.3 皮肤的字节数据
    ...
    public boolean isValid(int protocol) {
        if(protocol <= ProtocolInfo.v_1_0_0){
            return this.skinBytes != null &&  (this.skinBytes.length == SINGLE_SKIN_SIZE || this.skinBytes.length == DOUBLE_SKIN_SIZE);
        }
        return isValid(Server.getInstance().doNotLimitSkinGeometry);
    }

    private boolean isValid(boolean doNotLimitSkinGeometry) {
     ...
     * @param bytes
     */
    public void setSkinBytes(byte[] bytes){
        this.skinBytes = bytes;
    }
    /***
     * 0.14.3引入
     */
