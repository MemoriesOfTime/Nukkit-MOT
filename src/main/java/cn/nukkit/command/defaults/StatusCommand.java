    ...
    public boolean execute(CommandSender sender, String commandLabel, String[] args) {
        ...
        if (simpleMode) {
            ...
            for (Level level : server.getLevels().values()) {
                String worldInfo = TextFormat.GOLD + "World \"" + level.getFolderName() + "\"" + (!Objects.equals(level.getFolderName(), level.getName()) ? " (" + level.getName() + ")" : "") + ": " +
                ...
                if (level.isParallelTickEnabled()) {
                    ...
                    worldInfo += TextFormat.AQUA + " [Parallel TPS: " + levelTpsColor + NukkitMath.round(levelTps, 2) +
                            TextFormat.AQUA + " MSPT: " + TextFormat.YELLOW + NukkitMath.round(level.getLevelMSPT(), 2) + TextFormat.AQUA + "]";
                }
