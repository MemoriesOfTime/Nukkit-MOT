...
public final class BlockState implements Serializable, IBlockState {
    ...
    public static BlockState of(@NotNull String persistedStateId, boolean useDefaultPropertyValues) {
        ...
        int id;
        Optional<Integer> optionalId = Optional.ofNullable(BlockStateRegistry.getMapping(ProtocolInfo.CURRENT_PROTOCOL).getBlockId(namespacedId));
        if (optionalId.isEmpty()) {
            String fullId = BlockStateRegistry.getBlockMappings().inverse().get(persistedStateId);
            if (fullId != null) {
                List<String> sId = List.of(fullId.split(":"));
    ...
    public ItemBlock asItemBlock(int count) {
        ...
        int runtimeId = trimmedState.getRuntimeId(ProtocolInfo.CURRENT_PROTOCOL);
        if (runtimeId == BlockStateRegistry.getMapping(ProtocolInfo.CURRENT_PROTOCOL).getUpdateBlockRegistration() && !"minecraft:info_update".equals(trimmedState.getPersistenceName())) {
    ...
    public BlockProperties getProperties() {
        return BlockStateRegistry.getMapping(ProtocolInfo.CURRENT_PROTOCOL).getProperties(blockId);
    }
    ...
    public void validate() {
        if (valid.isPresent() && valid.getAsBoolean()) {
            return;
        }
        ...
        try {
            ...
            valid = OptionalBoolean.of(true);
        } catch (Exception e) {
            valid = OptionalBoolean.of(false);
    ...
    public Block getBlock(@Nullable Level level, int x, int y, int z, int layer, boolean repair, @Nullable Consumer<BlockStateRepair> callback) {
        ...
        Consumer<BlockStateRepair> updater = r -> valid = OptionalBoolean.of(false);
