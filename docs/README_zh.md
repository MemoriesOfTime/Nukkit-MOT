...
### Nukkit-MOT 的新特性
1. 支持 1.2 – 1.26.50 版本（可在配置中设置最低协议版本）
2. 支持大多数实体的AI
...
- 只想对外开放一个 UDP 端口时，设置 `server-udp-ports=19132`（即 server-port 本身）：NetherNet 媒体会在进程内从 RakNet 的 socket 中继到一个自动挑选（从 19134 起取第一个空闲）、只绑回环的内部端口，此时只需映射 `19132/udp` 与 `19132/tcp`，可去掉 `-p 19134:19134/udp`。`server-udp-ports=19132:19134` 效果相同，只是钉住了内部端口。
...
```xml
<repositories>
    <!-- 发布版来自 Maven Central；开发版 SNAPSHOT 需要添加 repo.lanink.cn 仓库 -->
    <repository>
...
repositories {
    mavenCentral()
    // 开发版 SNAPSHOT 需要 repo.lanink.cn 仓库
